package cn.zkz.common.redis.utils;

import cn.zkz.common.core.GlobalApplicationContext;
import cn.zkz.common.core.constants.CommonConstant;
import cn.zkz.common.core.exception.SysException;
import cn.zkz.common.core.utils.SafeEncoder;
import cn.zkz.common.core.utils.SerializationUtil;
...
public class RedisUtil {
    ...
    public void del(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                redisTemplate.delete(buildKey(CollectionUtils.arrayToList(key)));
            }
        }
    }
    ...
    public boolean setIfAbsent(String key, String value) {
        try {
            redisValueOperations.setIfAbsent(key, value);
            return true;
        } catch (Exception e) {
            log.error("", e);
            return false;
    ...
    public boolean setIfAbsent(String key, String value, long timeout) {
        try {
            redisValueOperations.setIfAbsent(key, value, timeout, TimeUnit.SECONDS);
            return true;
        } catch (Exception e) {
            log.error("", e);
            return false;
    ...
    public boolean setIfAbsentWithBuildKey(String key, String value) {
        try {
            redisValueOperations.setIfAbsent(buildKey(key), value);
            return true;
        } catch (Exception e) {
            log.error("", e);
            return false;
